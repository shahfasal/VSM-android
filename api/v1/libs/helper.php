...
function get_profile_id_from_secret($secret, $oauth) {
    $profile_id = null;
    if (isset($secret)) {
        include 'sql.php';
        $sql = "SELECT user.profile_id FROM " . $dbname . ".user,oauth where secret='$secret' and oauth_key='$oauth'";
    ...
    if (isset($username_user) && isset($password_user) && isset($device)) {
        ...
        if ($secret != 'false') {
            ...
            if ($profile_id != null && $oauth_key == null) {
                ...
                $res = get_userdata($profile_id);
                $result_array = array('status' => 'Success',
                    'message' => 'User found', 'number_of_devices_logged_in' => get_users_logged_in($profile_id), 'secret_key' => $secret, 'user_data' => $res);
...
function get_device_data($profile_id, $oauth) {
    include 'sql.php';
    $sql = "SELECT * FROM " . $dbname . ".oauth where profile_id=$profile_id and oauth_key='$oauth' ";
    //echo $sql;
    $result = $conn->query($sql);
    if (!$result) {
        print_r(sprintf("Error: %s", $conn->error));
    }
...
function get_oauth_from_fingerprint($fingerprint, $profile_id) {
    include 'sql.php';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($expiry_date >= $today) {
            $devices[] = $row['device'];
            $device = json_decode($row['device']);
            //print_r($device);
            $finger_print = $device->{'fingerprint'};
            if ($finger_print == $fingerprint) {
                return $row['oauth_key'];
            }
        }
    }
}

function logout_specific_device($profile_id, $fingerprint) {
    include 'sql.php';
    $sql = "SET foreign_key_checks = 0";
    $res = $conn->query($sql);
    $oauth_key = get_oauth_from_fingerprint($fingerprint, $profile_id);
    logout($oauth_key);
}
...
function my_file_meta($course_id, $file_path, $type) {
    include 'sql.php';


    if ($type == "video") {
        $sql = "insert into video_files(course_id,file_path) values($course_id,'$file_path')";
    }
    if ($type == "image") {
        $sql = "insert into image_files(course_id,file_path) values($course_id,'$file_path')";
    }
    $result = $conn->query($sql);
    return $result;
}
...
function check_enroll($course_id, $profile_id) {
    include 'sql.php';
    include 'sql.php';
    $sql = "SELECT profile_id FROM " . $dbname . ".enroll where course_id=$course_id and profile_id=$profile_id";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
