...
for ($x = 0; $x < count($logged_in_profiles); $x++) {
    ...
    if (isset($request->{'location'})) {
         ...
         */
        $myreq=$request->{'location'};
        if($myreq->{'city'}=="" && $myreq->{'region'}=="" && $myreq->{'country'}=="")
        {
            $flag=true;
        }
        else
...
//        print_r($location_response);
        ...
        if ($location_request->{'country'} != "" && $location_request->{'region'} != "" && $location_request->{'city'} != "") {
            if ($location_request->{'city'} == $location_response->{'city'} &&
                ...
                $flag = true;
                //print_r($location_request->{'city'});exit;
            } else {
                $flag = false;
...
//        print_r($location_response);
        ...
        if($location_request->{'logout_date'} == "" && $location_request->{'login_time'} == "" &&
        $location_request->{'login_date'}=="" && $location_request->{'logout_time'} == "" )
